...
export const CONTENT_NICHES = [
  ...
  { id: 'dentistry', label: 'Стоматология', tone: 'Страхи пациентов, до/после, гигиена, запись на осмотр.' },
  { id: 'cafe', label: 'Кафе и рестораны', tone: 'Блюда дня, атмосфера, акции, бронирование столика.' },
  { id: 'auto', label: 'Автосервис', tone: 'Диагностика, сезонное ТО, честные цены, запись.' },
];
...
const STUDIO_TEMPLATES = {
  ...
  services: [
    ...
    'Отзыв клиента своими словами',
    'FAQ: 5 вопросов до заявки',
    'Что будет на первой встрече',
    'Акция / спецпредложение недели',
    'Призыв записаться — мягко и по делу',
  ],
  shop: [
    'Товар дня: зачем он нужен',
    'Подборка для новичков',
    'Как выбрать размер / комплект',
    'Доставка и возврат — коротко',
    'Отзыв с фото от покупателя',
    'Скидка дня / промокод',
    'Что заказать вместе — комплект',
  ],
  realestate: [
    'Объект недели: планировка и плюсы района',
    'Ипотека: на что смотреть в первую очередь',
    'Частые ошибки покупателей',
    'Сравнение двух вариантов',
    'Как проходит показ',
    'Документы для сделки — чеклист',
    'Приглашение на консультацию',
  ],
  dentistry: [
    'Страх стоматолога: как мы работаем мягко',
    'Гигиена: зачем и как часто',
    'До/после лечения без фильтров',
    'Имплантация: этапы простыми словами',
    'Акция на осмотр / гигиену',
    'Ответы на вопросы в Direct',
    'Запись на удобное время',
  ],
  cafe: [
    'Блюдо дня: состав и история',
    'Атмосфера зала — для кого идеально',
    'Акция на завтраки / бизнес-ланч',
    'Как забронировать столик',
    'Отзыв гостя',
    'Сезонный напиток / десерт',
    'Приглашение на вечер',
  ],
  auto: [
    'Сезонное ТО: что проверить сейчас',
    'Признаки, что пора на диагностику',
    'Прозрачная смета — из чего складывается',
    'Кейс ремонта: до/после',
    'Акция на замену масла / колодок',
    'FAQ по гарантии',
...
export function studioPlanDrafts(nicheId, days = 7) {
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    const labeled = round > 0 ? `${title} (${round + 1})` : title;
    return {
      title: labeled,
